...
export default function BrokerPendingSalesPage() {
  const { currentUser } = useAppContext();
  const brokerId = currentUser?.companyId ?? "";
  const salesQuery = useBrokerSalesRequests(brokerId);
  const rows = useMemo(() => (salesQuery.data ?? []).filter((row) => row.status === "Pending"), [salesQuery.data]);
    ...
    <div className="space-y-5 pb-6">
      <section className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-[#dbe4eb] bg-white p-4">
        <div className="flex w-full max-w-[340px] items-center gap-2 rounded-md border border-[#dbe4eb] bg-white px-3 py-2.5">
          ...
          <input
            ...
            placeholder="Search request, property, project"
          />
        </div>
      </section>
      ...
      {salesQuery.error && (
        <section className="rounded-md border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
          {salesQuery.error instanceof Error ? salesQuery.error.message : "Unable to load pending sales requests."}
        </section>
      )}
      ...
      {selected && (
        <section className="rounded-md border border-[#dbe4eb] bg-white p-4">
          <h2 className="font-display text-lg text-[#1f2a44]">Sale Request Detail</h2>
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <label className="ui-label">Developer<input readOnly className="ui-input" value={selected.developerId} /></label>
            <label className="ui-label">Agreement<input readOnly className="ui-input" value={selected.agreementId} /></label>
            <label className="ui-label">Submitted by Agent<input readOnly className="ui-input" value={selected.brokerAgentId ?? "Broker Admin"} /></label>
