...
export default function BrokerProjectReportsPage() {
  ...
  const rows = useMemo(() => {
    ...
    return reports.projectRows.filter((row) => !q || [row.projectName, row.developerId, row.demandLevel].join(" ").toLowerCase().includes(q));
      ...
      <section className="rounded-md border border-[#dbe4eb] bg-white p-4">
        <div className="flex w-full max-w-[340px] items-center gap-2 rounded-md border border-[#dbe4eb] bg-white px-3 py-2.5">
          ...
          <input
            ...
            placeholder="Search project report"
          />
        </div>
      </section>
