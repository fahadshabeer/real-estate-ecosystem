...
export default function BrokerAgentReportsPage() {
  ...
  const rows = useMemo(() => {
    ...
    return reports.agentRows.filter((row) => !q || [row.name, row.agentId, row.status].join(" ").toLowerCase().includes(q));
  }, [reports.agentRows, search]);
