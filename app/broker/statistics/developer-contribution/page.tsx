...
export default function BrokerDeveloperContributionPage() {
  ...
  const rows = useMemo(() => {
    ...
    return analytics.developerRows.filter((row) => !q || row.developerId.toLowerCase().includes(q));
  }, [analytics.developerRows, search]);
  ...
  const chartData = rows.slice(0, 8).map((row) => ({
    ...
    contribution: Number(row.revenueContribution.toFixed(1)),
  }));
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      <section className="rounded-md border border-[#dbe4eb] bg-white p-4">
        <h2 className="text-base font-semibold text-[#1f2a44]">Developer Revenue Contribution</h2>
          ...
          <table className="min-w-full text-left text-sm text-[#4f6078]">
            <thead className="bg-[#f8fafc] text-xs uppercase tracking-[0.12em] text-[#7f8a99]">
              <tr>
                ...
                <th className="px-4 py-3">Sales Closed</th>
              ...
              {!isInitialLoading &&
                pageRows.map((row, index) => (
                  <tr key={row.developerId} className="border-t border-[#ecf1f5]">
                    ...
                    <td className="px-4 py-3 font-medium text-[#1f2a44]">{row.salesClosed}</td>
                    <td className="px-4 py-3">{percent(row.revenueContribution)}</td>
              ...
              {!isInitialLoading && rows.length === 0 && (
                <tr className="border-t border-[#ecf1f5]">
                  <td className="px-4 py-6 text-[#7f8a99]" colSpan={6}>
                    No developer contribution records found.
                  </td>
                </tr>
              )}
            </tbody>
