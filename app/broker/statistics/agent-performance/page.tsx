...
export default function BrokerAgentPerformanceAnalyticsPage() {
  ...
  const rows = useMemo(() => {
    ...
    return analytics.agentRows.filter((row) => !q || [row.name, row.agentId, row.status].join(" ").toLowerCase().includes(q));
  }, [analytics.agentRows, search]);
            ...
            <tbody>
              ...
              {!isInitialLoading &&
                pageRows.map((row, index) => (
                  <tr key={row.agentId} className="border-t border-[#ecf1f5]">
                    <td className="px-4 py-3">
                      <p className="font-medium text-[#1f2a44]">{row.name}</p>
                      <p className="text-xs text-[#7f8a99]">{row.agentId}</p>
                    </td>
                    <td className="px-4 py-3">{row.assignedUnits}</td>
                    <td className="px-4 py-3">{row.submittedSales}</td>
                    <td className="px-4 py-3 font-medium text-[#1f2a44]">{row.approvedSales}</td>
                    <td className="px-4 py-3">
