...
const roles = ["Super Admin", "Sales Manager", "Agent Coordinator", "Analyst"] as const;

export default function BrokerUsersRolesSettingsPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", role: roles[1] as (typeof roles)[number], phone: "", password: "" });
  const [formError, setFormError] = useState<string | null>(null);

  const roleMatrix = useMemo(
    () => [
      { role: "Super Admin", access: "Full access across all broker modules" },
      { role: "Sales Manager", access: "Sales, inventory, analytics, reports" },
      { role: "Agent Coordinator", access: "Agents, assignments, performance" },
      { role: "Analyst", access: "Analytics and reports read-only" },
    ],
    [],
  );
    ...
    <div className="space-y-5 pb-6">
      ...
      <section className="rounded-md border border-[#dbe4eb] bg-white p-5">
        <div className="mb-4 flex items-center justify-between">
          <p className="text-base font-semibold text-[#1f2a44]">Internal Staff Access</p>
          <button type="button" onClick={() => setShowForm((prev) => !prev)} className="inline-flex items-center gap-2 rounded-md bg-[#3aa4a8] px-4 py-2 text-sm font-semibold text-white">
            <UserPlus className="h-4 w-4" />
            Add Internal User
          </button>
        </div>

        <p className="mb-3 text-xs text-[#607187]">Agents are managed separately in the Agents module and are not internal dashboard staff users.</p>
        ...
        {showForm ? (
          <div className="mb-4 grid gap-3 rounded-md border border-[#dbe4eb] bg-[#f8fafc] p-4 md:grid-cols-2">
            ...
            <label className="text-sm text-[#4f6078]">
              Role
              <FancySelect value={form.role} onChange={(e) => setForm((p) => ({ ...p, role: e.target.value as (typeof roles)[number] }))} className="mt-1 h-11 w-full rounded-md border border-[#dbe4eb] bg-white px-3 text-sm text-[#1f2a44]">
                {roles.map((role) => (
            ...
            <div className="md:col-span-2">
              <button type="button" onClick={addUser} disabled={createInternalUser.isPending} className="inline-flex items-center gap-2 rounded-md border border-[#dbe4eb] bg-white px-4 py-2 text-sm font-medium text-[#1f2a44]">
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.uid} className="border-t border-[#ecf1f5]">
                  <td className="px-4 py-3"><p className="font-medium text-[#1f2a44]">{user.name}</p><p className="text-xs text-[#7f8a99]">{user.email}</p></td>
              ...
              {users.length === 0 ? (
                <tr className="border-t border-[#ecf1f5]"><td className="px-4 py-6 text-[#7f8a99]" colSpan={5}>No users added yet.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
