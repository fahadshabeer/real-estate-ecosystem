...
export default function ApprovedSalesPage() {
  ...
  const rows = useMemo(
    () => (requestsQuery.data ?? []).filter((row) => row.status === "Approved"),
    [requestsQuery.data],
  );
      ...
      <section className="overflow-hidden rounded-md border border-[#dbe4eb] bg-white">
        ...
        <div className="min-h-[500px] overflow-x-auto">
          <table className="min-w-full text-left text-sm text-[#4f6078]">
            ...
            <tbody>
              {requestsQuery.isLoading && <TableSkeletonRows cols={7} rows={8} />}
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-[#ecf1f5]">
                  ...
                  <td className="px-4 py-3">{row.approvedAt ? new Date(row.approvedAt).toLocaleString() : "—"}</td>
                  <td className="px-4 py-3">{row.brokerAgentId ?? "AGT-PENDING"}</td>
                </tr>
              ))}
