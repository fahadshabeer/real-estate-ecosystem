...
export default function BrokerPerformanceAnalyticsPage() {
  ...
  const rows = useMemo(() => {
    ...
    return analytics.brokerPerformanceRows.filter((row) => {
      if (!q) return true;
      return [row.brokerId, row.responseSpeed].join(" ").toLowerCase().includes(q);
    });
  }, [analytics.brokerPerformanceRows, search]);
  ...
  const avgApproval = rows.length > 0 ? rows.reduce((sum, row) => sum + row.approvalSuccess, 0) / rows.length : 0;
  const avgConversion = rows.length > 0 ? rows.reduce((sum, row) => sum + row.conversionRate, 0) / rows.length : 0;
  const totalDeals = rows.reduce((sum, row) => sum + row.dealsClosed, 0);

  const chartData = rows.slice(0, 6).map((row) => ({
    broker: row.brokerId,
    deals: row.dealsClosed,
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {[
          ["Active Brokers", String(rows.length)],
          ["Deals Closed", String(totalDeals)],
          ["Approval Success", percent(avgApproval)],
          ["Avg Conversion", percent(avgConversion)],
        ].map(([label, value]) => (
      ...
      <section className="rounded-md border border-[#dbe4eb] bg-white p-4">
        <h2 className="text-base font-semibold text-[#1f2a44]">Broker Leaderboard by Performance</h2>
        <div className="mt-4 h-[300px] w-full">
          {analytics.loading ? (
            ...
            <div className="grid h-full w-full place-items-center rounded-md border border-dashed border-[#dbe4eb] text-sm text-[#7f8a99]">
              No broker leaderboard data yet.
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={220}>
              <BarChart data={chartData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e6edf3" />
                <XAxis dataKey="broker" tick={{ fill: "#607187", fontSize: 11 }} />
                <YAxis tick={{ fill: "#607187", fontSize: 11 }} />
                <Tooltip
                  contentStyle={{ borderRadius: 8, border: "1px solid #dbe4eb", backgroundColor: "#fff" }}
                />
                <Bar dataKey="deals" fill="#3aa4a8" radius={[6, 6, 0, 0]} />
                <Bar dataKey="trust" fill="#71b9c2" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
              ...
              {!analytics.loading &&
                pageRows.map((row) => (
                  <tr key={row.brokerId} className="border-t border-[#ecf1f5]">
                    ...
                    <td className="px-4 py-3 font-medium text-[#1f2a44]">{row.dealsClosed}</td>
                    <td className="px-4 py-3">{percent(row.approvalSuccess)}</td>
                    <td className="px-4 py-3">{percent(row.conversionRate)}</td>
                    <td className="px-4 py-3">{row.responseSpeed}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <Link
                        href={`/portal/brokers/${encodeURIComponent(row.brokerId)}`}
                        className="inline-flex rounded-lg border border-[#dbe4eb] bg-[#f8fafc] p-2 text-[#4f6078]"
                        aria-label="Open broker profile"
