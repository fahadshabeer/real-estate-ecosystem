...
export default function CompanyProfileSettingsPage() {
  ...
  const logoPreview = logoFile
    ...
    : companyQuery.data?.logoUrl ?? null;
  const sealPreview = sealFile
    ? URL.createObjectURL(sealFile)
    : settingsQuery.data?.profileExtra.sealDataUrl ?? null;
    ...
    <div className="space-y-5 pb-6">
      ...
      <section className="rounded-md border border-[#dbe4eb] bg-white p-5">
        <div className="mb-4 grid gap-3 md:grid-cols-3">
          {[{ label: "Company Logo", preview: logoPreview, set: setLogoFile }, { label: "Company Seal", preview: sealPreview, set: setSealFile }, { label: "Cover Image", preview: coverPreview, set: setCoverFile }].map((item) => (
        ...
        <div className="mt-4 flex items-center justify-end gap-2">
          {loading && <Loader2 className="h-4 w-4 animate-spin text-[#607187]" />}
          <button
            type="button"
            onClick={save}
        ...
        {!loading && companyId && (
          <p className="mt-3 inline-flex items-center gap-1 text-xs text-[#2f7f48]">
            ...
            Company identity synced for contracts, reports and legal references.
          </p>
        )}
      </section>
      )}
