...
export default function BrandingSettingsPage() {
  ...
  const [footerBranding, setFooterBranding] = useState(
    settings?.branding.footerBranding ?? "Confidential Developer Network",
  );
  ...
  const watermarkPreview = watermarkFile
    ? URL.createObjectURL(watermarkFile)
    : settings?.branding.watermarkDataUrl ?? null;
  const signaturePreview = signatureFile
    ? URL.createObjectURL(signatureFile)
    : settings?.branding.signatureDataUrl ?? null;
    ...
    <div className="space-y-5 pb-6">
      ...
      <section className="rounded-md border border-[#dbe4eb] bg-white p-5">
        <div className="grid gap-3 md:grid-cols-3">
          <label className="text-sm text-[#4f6078]">
            ...
            <input
              ...
              className="mt-1 h-11 w-full rounded-md border border-[#dbe4eb] bg-white px-2"
            />
          </label>
          <label className="text-sm text-[#4f6078]">
            Secondary Color
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {[
            { label: "Contract Watermark", preview: watermarkPreview, set: setWatermarkFile },
        ...
        <div className="mt-5 rounded-md border border-[#dbe4eb] bg-[#f8fafc] p-4">
          <p className="text-sm font-semibold text-[#1f2a44]">Live Contract Preview</p>
          <div className="mt-3 rounded-md border border-[#dbe4eb] bg-white p-4">
            <div className="relative overflow-hidden rounded-md border border-[#dbe4eb] p-4">
              ...
              <p className="mt-2 text-xs text-[#607187]">
                Jurisdiction and legal body preview with configured branding colors.
              </p>
        ...
        <p className="mt-2 inline-flex items-center gap-1 text-xs text-[#2f7f48]">
          ...
          Branding is applied to reports and legal PDF generation flow.
        </p>
      </section>
      )}
    </div>
