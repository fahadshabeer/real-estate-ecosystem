...
export default function ProjectReportsPage() {
  ...
  const rows = useMemo(() => {
    ...
    return reports.projectRows.filter((row) => q.length === 0 || row.projectName.toLowerCase().includes(q));
  }, [reports.projectRows, search]);
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            onClick={async () => {
              try {
                await runWithToast({
                  loading: "Generating project performance PDF...",
                  success: "Project report generated.",
                  action: () =>
          ...
          <table className="min-w-full text-left text-sm text-[#4f6078]">
            <thead className="bg-[#f8fafc] text-xs uppercase tracking-[0.12em] text-[#7f8a99]">
              <tr>
                ...
                <th className="px-4 py-3">Broker Participation</th>
              </tr>
            </thead>
            <tbody>
              ...
              {!reports.loading && pageRows.map((row) => (
                <tr key={row.projectId} className="border-t border-[#ecf1f5]">
                  ...
                  <td className="px-4 py-3">{row.brokerParticipation}</td>
                </tr>
              ))}
              {!reports.loading && rows.length === 0 && (
                <tr className="border-t border-[#ecf1f5]"><td className="px-4 py-6 text-[#7f8a99]" colSpan={5}>No project report data yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
